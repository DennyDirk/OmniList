...
import { dictionaries, formatDateTime, formatPublishJobStatus, type Locale } from "../lib/i18n";
...
export function PublishJobHistory({ jobs, locale }: { jobs: PublishJob[]; locale: Locale }) {
  ...
  function renderJob(job: PublishJob) {
    return <article className="list-item" key={job.id}>
      ...
      {job.targets.map(target => <div key={target.id}><strong>{target.channelName}</strong><p className="listing-result-message">{target.message}</p></div>)}
    </article>;
  }
