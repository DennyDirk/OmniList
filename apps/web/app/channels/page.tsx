...
import { etsyConnectionError } from "../../lib/etsy-copy";
...
export default async function ChannelsPage({ searchParams }: {
  ...
  return <main className="shell listing-shell">
    <header className="listing-header">
      <Link className="text-link" href="/">{publishCopy[locale].catalog}</Link>
      <h1>{storeCopy[locale].title}</h1>
      <p>{storeCopy[locale].intro}</p>
      <p className="field-hint">{storeCopy[locale].availability}</p>
    </header>
